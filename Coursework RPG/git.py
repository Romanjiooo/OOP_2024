import pygame
import sys
import json  # Используется для сохранения данных в формате JSON
import os
import random
...
class Character:
    def __init__(self, screen):
        self.screen = screen
        self.image = pygame.transform.scale(pygame.image.load("main_character.png"), (50, 100))
        self.pos = [50, 300]
        self.rect = self.image.get_rect(topleft=self.pos)  # Initialize the rect

    def draw(self):
        self.screen.blit(self.image, self.rect)  # Use rect for positioning

    def move(self, dx, dy):
        self.pos[0] += dx
        self.pos[1] += dy
        self.rect.topleft = self.pos  # Update the rect when the character moves
...
class Game:
    ...
    def update(self):
        ...
        if abs(self.character.pos[0] - self.door_pos[0]) < 50 and abs(self.character.pos[1] - self.door_pos[1]) < 100:
            self.at_door = True
        else:
            self.at_door = False
...
class Inventory:
    ...
    def draw(self):
        # Рисование инвентаря и полосы здоровья
        x_start = 200
        y_start = 500
        icon_size = 40
    ...
    def reset_game(self):
        ...
        self.health = 1000
        self.items = []
        return 'restart'  # Сигнал о перезапуске игры
...
class Combat:
    ...
    def attack(self, enemy):
        ...
        if enemy in self.enemies:
            damage = 0.00000001  # Пример значений
            enemy.health -= damage
...
class EnemySpawner:
    ...
    def spawn_enemies(self):
        for _ in range(self.num_enemies):
            # Рандомная позиция спавна в пределах экрана
    ...
    def check_collisions(self, target_pos, damage_range):
        ...
        for enemy in self.enemies:
            if enemy.check_collision(target_pos, damage_range):
                damage += 5  # Каждый враг наносит 5 урона
        return damage
...
class Enemy:
    ...
    def update(self, target_pos):
        # Простое движение к персонажу: преследование
...
class NPC:
    ...
    def interact(self):
        if self.dialogue_index < len(self.dialogue):
            print(self.dialogue[self.dialogue_index])  # Пример вывода диалога
...
class Door:
    def __init__(self, screen, image_path, position, rect, to_scene, scale):
        ...
        self.image = pygame.transform.scale(self.image, scale)  # Изменение размера изображения
        self.position = position
        self.rect = self.image.get_rect(topleft=rect)
        self.to_scene = to_scene  # Сцена, куда ведет эта дверь
...
class Wall:
    ...
    def draw(self, screen):
        pygame.draw.rect(screen, GREY, self.rect)  # Стены можно нарисовать серым цветом для наглядности
...
enemy_spawner = EnemySpawner(screen, "enemy_icon.png", 5)  # Путь к иконке врага
...
door_scale = (100, 200)  # Новый размер для изображения двери
doors = [
...
while True:
    for event in pygame.event.get():
        ...
        if current_scene == "menu":
            ...
            if result == "start":
                current_scene = "game"  # Переход в игровую сцену
            elif result == "quit":
                pygame.quit()
                sys.exit()
                ...
                if event.key == pygame.K_SPACE:
                    for npc in npcs:
                        if npc.rect.collidepoint(game.character.pos):
                            npc.interact()  # Взаимодействие с NPC
                for door in doors:
                    if door.rect.colliderect(game.character.rect):
                        current_scene = door.to_scene  # Переход через дверь
    ...
    if current_scene == "game":
        game.update()
        enemy_spawner.update_enemies(game.character.pos)
        damage = enemy_spawner.check_collisions(game.character.pos, 50)
    ...
    if current_scene == "menu":
        ...
        for door in doors:
            door.draw(screen)  # Отрисовка дверей
    elif current_scene == "esc":
        esc_menu.draw()

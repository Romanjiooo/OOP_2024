...
class Star:
    ...
    def create_planets(self):
        for i in range(self.num_planets):
            ...
            orbit_radius = ORBIT_SPACING * (i // 3 + 1)
            clockwise = (i % 2 == 0)
            has_satellite = False

            # Проверка условия для спутников
            if self.star_index in [0, 2] and ((i // 3 + 1) % 2 == 0):
                has_satellite = True
...
class Satellite:
    ...
    def create_satellite(self):
        x = self.planet_x + self.radius * 3 * math.cos(math.radians(self.angle))
        y = self.planet_y + self.radius * 3 * math.sin(math.radians(self.angle))
